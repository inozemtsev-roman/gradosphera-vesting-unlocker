...
export function WalletConnector({ onAddressSet, isTestnet }: WalletConnectorProps) {
  ...
  const handleConnect = async () => {
    try {
      ...
      console.error('Ошибка открытия кошелька:', error);
    }
  };
  ...
  const validateAndFormatAddress = async (address: string): Promise<string | null> => {
    try {
      ...
      if (!exists) {
        setAddressError('По этому адресу нет депозитов');
        return null;
      }
      
      return nonBounceable;
    } catch (error) {
      setAddressError('Неправильный формат адреса');
      return null;
    }
  };
  ...
  return (
    <div className="w-full max-w-md mx-auto">
      <div className="bg-white rounded-2xl shadow-lg p-8">
        <h1 className="text-3xl font-bold text-center mb-2">Разблокировка вестинга</h1>
        <p className="text-gray-500 text-center mb-8">
          Разблокируйте ваши TON из контракта
          {isTestnet && <span className="text-orange-500 ml-2">(Тестовая сеть)</span>}
        </p>
        ...
        {!showManualInput ? (
          ...
          <form onSubmit={handleManualSubmit} className="space-y-4">
            ...
            <div className="flex space-x-3">
              ...
              >
                {isValidating ? 'Проверка...' : 'Продолжить'}
              </button>
            </div>
          </form>
        )}
